...
export default function Checkbox({ label = "", className = "", name, ...props }: Checkbox) {
	return (
		<label
			...
			id={name}
		>
			<input type="checkbox" className="sr-only peer" name={name} {...props} />
			<div className="flex items-center justify-center size-20 max-lg:size-17 max-md:size-14 max-sm:size-13 max-xs:size-12 rounded-[0.6rem] max-lg:rounded-[0.5rem] max-md:rounded-[0.4rem] max-sm:rounded-[0.35rem] max-xs:rounded-[0.3rem] border-1 text-icon-purple overflow-hidden border-icon-blue peer-checked:border-icon-purple *:hidden peer-checked:*:block hover:border-typo-light-white duration-200 peer-checked:hover:border-icon-purple peer-focus:border-icon-purple">
				<Check className="size-full bg-typo-light-white" />
			</div>
			{label}
		</label>
	);
