...
export default function Prompt() {
	...
	const send = () => {
		dispatch({ type: "SET_IS_MODEL_GENERATING", payload: true });
	};
	return (
		<div className="flex gap-2 max-md:gap-1 mx-auto w-full">
			...
			<button
				onClick={send}
				className="bg-btn-dark hover:bg-glass-white rounded-r-[1.2rem] max-lg:rounded-r-[1rem] max-md:rounded-r-[0.8rem] max-sm:rounded-r-[0.6rem] max-xs:rounded-r-[0.5rem] cursor-pointer px-16 max-lg:px-14 max-md:px-10 max-sm:px-8 max-xs:px-6 disabled:text-typo-dark-gray disabled:cursor-not-allowed disabled:hover:bg-btn-dark *:size-24 max-lg:*:size-20 max-md:*:size-18 max-sm:*:size-16 max-xs:*:size-14 duration-300 active:brightness-50"
			>
				<Send />
			</button>
		</div>
